// 날짜 및 시간 등록 정보Dlg.cpp : implementation file
//

#include "stdafx.h"
#include "날짜 및 시간 등록 정보.h"
#include "날짜 및 시간 등록 정보Dlg.h"
...
BOOL CMyDlg::OnInitDialog()
{
	...
	COleDateTime mm;
	mm = COleDataTime::GetCurrentTime();
	m_nYear=mm.GetYear();
...
void CMyDlg::OnVScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar) 
{
	...
	{
	case SB_LINELEFT:
		if(m_nYear<3000) //년도 최대값.
		{
			m_nYear++;
		}
		break;
	case SB_LINERIGHT:
		if(m_nYear>1900) //년도 최소값.
		{
			m_nYear--;
		}
		break;
...
void CMyDlg::OnSelchangeComboMonth()  //콤보상자의 데이터를 선택
{
