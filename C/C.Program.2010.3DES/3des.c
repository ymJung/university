...
void main()
{
	...
	for(i=0;i<block_count;i++)
		DES_Encryption(&p_text[i*BLOCK_SIZE],&c1_text[i*BLOCK_SIZE],key1);
	...
	msg_len=(int)strlen((char*)c1_text);
	block_count=(msg_len%BLOCK_SIZE)?(msg_len/BLOCK_SIZE+1):(msg_len/BLOCK_SIZE);
	
	for(i=0;i<block_count;i++) //3DES Cipher
		DES_Encryption(&c1_text[i*BLOCK_SIZE],&c2_text[i*BLOCK_SIZE],key2);
	...
	msg_len=(int)strlen((char*)c2_text);
	block_count=(msg_len%BLOCK_SIZE)?(msg_len/BLOCK_SIZE+1):(msg_len/BLOCK_SIZE);
	

	for(i=0;i<block_count;i++)
		DES_Encryption(&c2_text[i*BLOCK_SIZE],&c3_text[i*BLOCK_SIZE],key1);
	...
	//Decryption
	for(i=0;i<block_count;i++)
		DES_Decryption(&c1_text[i*BLOCK_SIZE], &d_text[i*BLOCK_SIZE],key1);
...
void DES_Decryption(BYTE *c_text,BYTE *result,BYTE *key)
{
	...
	for(i=0;i<DES_ROUND;i++)
	{
		L=L^f(R,round_key[DES_ROUND-i-1]); //복호화시 라운드기를 역순으로 적용
