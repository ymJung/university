public class SHA512 {
	...
	private final synchronized long[] sha(long hh[], byte[] in) //객체단위 쓰레드 동기화
	{
	 ...
	 public static void main(String[] args){
		 ...
		 //str = "ABCDEFGHIJUVWXYZ01VWXYZ0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789ABCDEFGHIJKLMNOPQRSTUAAAAAAAB";
		 str = "A";
		...
		//마지막 블럭 
		 for(int i=0;i<roop;i++){ 
			 ...
			 for(int mp=0;mp<BLOCK_SIZE;mp++)
				 System.out.print(msgbyte[mp]+" ");
			 HashProcess = Sha.sha(HashProcess, msgbyte); //메시지 블럭을 처리
			 for(int j=0;j<8;j++)
				 System.out.format("%02X%n",HashProcess[j]);
		 }
		 
		 System.out.println("마지막블럭과 해쉬시킬 해쉬값");
		 ...
		 msglength[15] = (byte)str.length(); // 16바이트 전체에 숫자를 넣을수 있게 수정요망
		 
		 for(int i=0;i<=15;i++) //메시지+패딩+메시지 원래 길이를 붙임
			 msgbyte[112+i] = (byte)msglength[i]; 
