public class SHA512 {
	...
	private long[][] str2long(int roop,int cnt,String str){ //str을 long으로 바꿈
		long[][] msgblock = new long[roop][cnt];
		
		for(int i=0;i<roop;i++) 
			 for(int j=0;j<BLOCK_SIZE;j++)
				 msgblock[i][j] = (long)str.charAt((i*BLOCK_SIZE)+j); //스트링을 롱데이터형으로 바꿈
	...
	private void printmsgblock(int roop,int cnt,long[][] msgblock){ //전체 메시지블럭 출력
		for(int i=0;i<roop;i++)
			 for(int j=0;j<cnt;j++)
	...
	private byte[] lastmsgblock(byte[] padresult, int cnt,String str,int roop){ //마지막 블럭 설정 : 마지막 메시지 + 패딩 + 메시지길이
		...
		msgbyte[120] = (byte)(lmsglength >>56);
		msgbyte[121] = (byte)(lmsglength >>48);
		msgbyte[122] = (byte)(lmsglength >>40);
		msgbyte[123] = (byte)(lmsglength >>32);
		msgbyte[124] = (byte)(lmsglength >>24);
		msgbyte[125] = (byte)(lmsglength >>16);
		msgbyte[126] = (byte)(lmsglength >>8);
		msgbyte[127] = (byte)(lmsglength);
	...
	public static void main(String[] args){
		 ...
		 SHA512 Sha = new SHA512(); //메인 함수 선언
		 
		 String str = new String();
		 str = "";
		 //str = "A";
		 ...
		 msgblock = Sha.str2long(roop,cnt,str);  //스트링을 롱데이터형으로 바꿈
