...
public class SHA512 {
	...
	private byte[][] str2byte(int roop,String str){ //str을 byte으로 바꿈
		byte[][] msgblock = new byte[roop][BLOCK_SIZE];
		
		for(int i=0;i<roop;i++){ 
			 for(int j=0;j<BLOCK_SIZE;j++) // 메시지 수가 112~128 사이 일때 뒤를 0으로 채우고 다음 블럭으로 넘어갈것
				 msgblock[i][j] = (byte)str.charAt((i*BLOCK_SIZE)+j); //스트링을 바이트형으로 바꿈
			 
		}
		return msgblock;
	}
	//패딩
	protected byte[] padBuffer(int cnt) {
		int n = cnt % BLOCK_SIZE; // 원래 메시지 구역을 설정
		int padding;
		if(n>112) // 패딩공간이 112 보다 작다면  (16은 메시지 길이 저장공간)
			padding = 112-n; //패딩은 112에서 원래 메시지 길이 공간을 뺌
		else //아니면
			padding = 240-n; // 112보다 크다면 다음블럭으로 넘어가야함  
		byte[] result = new byte[padding]; //마지막 블럭의 길이 선언 
		result[0] = (byte) 0x80; //패딩 첫자리를 1로 선언 1000 0000 = -128
	...
	private String filein(String filename)throws IOException{
		...
		filestring = filestring.substring(4); //버퍼리더로 읽기때문에 초기 null값도 함께 읽어온다. 그 값을 빼줌
	...
	public static void main(String[] args){
		try{ 
			...
			msgblock = Sha.str2byte(roop,str);  //스트링을 롱데이터형으로 바꿈
